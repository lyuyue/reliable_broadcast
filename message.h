...
struct AckRecord {
    ...
    struct AckRecord *next; // ptr to next AckRecord
};

struct AckRecordHeader {
    uint32_t seq_ack_count; // counter of SeqAckMessage if SeqMessage of this DataMessage is sent
    uint32_t max_seq;       // final sequence
    uint32_t max_proposer;  // final sequence proposer
    struct AckRecord list;  // list header to AckRecord list of expected hosts
};
